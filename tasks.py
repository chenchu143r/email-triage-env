...
def grade_task_1(eid,cl):
    ...
    if eid not in exp: return 0.0
    if cl==exp[eid]: return 1.0
    if exp[eid] in ['urgent','normal'] and cl in ['urgent','normal']: return 0.3
    return 0.0
def grade_task_2(items):
    ...
    for g in groups:
        if any(any(k in item for k in g) for item in low): score+=1.0/len(groups)
    return round(min(score,1.0),2)
def grade_task_3(draft):
    if not draft or len(draft)<50: return 0.0
    d=draft.lower()
    kmap={'apologize':['apologize','sorry','apologies'],'acknowledge':['acknowledge','understand','outage','downtime'],'resolve':['resolve','fix','address','investigate'],'compensation':['compensation','compensate','refund','credit'],'contact':['contact','reach','call','discuss']}
    found=sum(1 for kws in kmap.values() if any(k in d for k in kws))
    score=found/len(kmap)
    if len(draft)>200: score=min(score+0.1,1.0)
    if any(g in d for g in ['dear','hello','hi ']): score=min(score+0.05,1.0)
